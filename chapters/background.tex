...
\paragraph{}SLAM systems use sensors such as \ac{LiDAR}'s, cameras, IMU's and GPS to collect data about the environment, which then is processed by the backend~\cite{taheri2021slam}, which is implementation dependent.
...
\end{algorithmic}
\end{algorithm}
\caption{Bayesian Optimization algorithm steps~\cite{brochu2010tutorial}.}
\label{bo_algo}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{A graphic representation of the simulated annealing algorithm~\cite{sa_netlogo}.}
    \label{fig:enter-label}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Hyperband described in a more systematic way~\cite{li2018hyperband}.}
    \label{fig:enter-label}
\end{figure}
...
\begin{tabular}{@{}cccc@{}}
\toprule
Article(s)                                                                                                          & SLAM category                                                                           & Optimization method                                 & Results                                                                                                                                                                                            \\ \midrule
\multicolumn{1}{|c|}{Z. Zheng (2020)~\cite{zheng2020feature}}                                 & \multicolumn{1}{c|}{Feature-based Visual Odometry(not full \ac{SLAM})} & \multicolumn{1}{c|}{Grid Search}                    & \multicolumn{1}{c|}{Across 8 optimized sequences, average ATE decreased, on average, 64.68\%}                                                                                                      \\ \midrule
\multicolumn{1}{|c|}{A. Putra and P. Prajitno (2019)~\cite{putra2019parameter}}               & \multicolumn{1}{c|}{G-mapping SLAM(particle filter)}                                    & \multicolumn{1}{c|}{Brute Force}                    & \multicolumn{1}{c|}{Navigation time of a predefined path was reduced from 32 to 25 seconds}                                                                                                        \\ \midrule
\multicolumn{1}{|c|}{K. Koide et al (2021)~\cite{koide2021automatic}}                         & \multicolumn{1}{c|}{Lidar Odometry(not full \ac{SLAM})}                & \multicolumn{1}{c|}{based on Bayesian Optimization} & \multicolumn{1}{c|}{\textbf{With data augmentation}, in both tested environments, both components (translational and rotational) of the drift error decreased by at least 9.8\%.} \\ \midrule
\multicolumn{1}{|c|}{A. Kostusiak and P. Skrzypczyński (2019)~\cite{kostusiak2019efficiency}} & \multicolumn{1}{c|}{RGB-D Visual Odometry}                                              & \multicolumn{1}{c|}{Particle Swarm Optimization}    & \multicolumn{1}{c|}{Both ATE and RPE decreased by well over 50\%, and optimized parameters generalize well to other sequences}                                                                     \\
A. Kostusiak and P. Skrzypczyński (2019)~\cite{kostusiak2019efficiency}                       & RGB-D Visual Odometry                                                                   & Evolutionary Algorithm                              & With \ac{EA}, the tuning duration is about 5x shorter than \ac{PSO},                                                                                             \\ \bottomrule
\end{tabular}
\caption{}
\label{tab:my-table}
\end{table}
